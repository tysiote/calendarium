...
function deleteEvent(item) {
    var e = events.get(parseInt($(item).prop("id")));
    var result = '<p>Naozaj chcete zmazať túto udalosť?</p>';
    result += e.title;
...
function deleteEvent2(hard) {
    var d = new Date();
    var temp = { action: "delete_event_soft", data: { id: deleting_event.id } };
    var soft = d.getFullYear() === deleting_event.start_time.year && d.getDate() === deleting_event.start_time.day && d.getMonth() + 1 === deleting_event.start_time.month;
    if (hard) soft = false;
    if (!soft) {
...
function populateEditInputs() {
    var e = editing_event;
    if (e.tags1.indexOf("sport") === -1) hideSport();else showSport();
    if (e.tags3.indexOf("all") === -1) hideArea();else showArea();
    $("#edit-event input").each(function (i, v) {
        $(v).prop("checked", false);
        $(v).val("");
    });
    if (e.tags1 && e.tags1.length) e.tags1.split("|").forEach(function (t) {
        $("#input-checkbox-" + t + "-edit").prop("checked", true);
    });
    if (e.tags2 && e.tags2.length) e.tags2.split("|").forEach(function (t) {
        $("#input-checkbox-tag-" + t + "-edit").prop("checked", true);
    });
    if (e.tags3 && e.tags3.length) e.tags3.split("|").forEach(function (t) {
        $("#input-checkbox-area-" + t + "-edit").prop("checked", true);
    });
    if (e.tags4 && e.tags4.length) e.tags4.split("|").forEach(function (t) {
        $("#input-checkbox-form-" + t + "-edit").prop("checked", true);
    });
    if (e.tags5 && e.tags5.length) e.tags5.split("|").forEach(function (t) {
        $("#input-checkbox-sport-" + t + "-edit").prop("checked", true);
    });
    if (e.sport_type && e.sport_type.length) e.sport_type.split("|").forEach(function (t) {
        $("#input-checkbox-sport-" + t + "-edit").prop("checked", true);
    });
    var content = customReplace(customReplace(e.content, "&nbsp;", " "), "<br>", "\n");
    var title = customReplace(customReplace(e.title, "&nbsp;", " "), "<br>", "\n");
    if (content && content.length) $("#input-content-edit").val(content);else $("#input-content-edit").val("");
    if (title && title.length) $("#input-title-edit").val(title);
    if (editing_event.no_time) {
        showDates(2);
        $("#input-add-no_time-edit").prop("checked", true);
    } else {
        showDates(1);
        $("#input-add-no_time-edit").prop("checked", false);
    }
    var d = e.start_time;
    $("#datetimepicker1a-edit").data("DateTimePicker").date(new Date(d.year, d.month - 1, d.day, d.hour, d.minute));
    $("#datetimepicker1b-edit").data("DateTimePicker").date(new Date(d.year, d.month - 1, d.day));
}
...
function grabCheckboxes(edit) {
    ...
    var date = '';
    if (edit) {
        if ($("#input-edit-no_time").prop("checked")) date = $("#input-date-2-edit").val();
        else date = $("#input-date-1-edit").val();
    } else {
        if ($("#input-add-no_time").prop("checked")) date = $("#input-date-2").val();
        else date = $("#input-date-1").val();
    }
    var tags1 = {
    ...
    if ($("#input-checkbox-area-all" + edit).is(":checked")) for (let key in tags3) tags3[key] = true;
    var tags4 = {
...
function editEventBeforeSend(data, adding, no_time) {
    var e = editing_event;
    if (adding) {
        adding_data = {};
        e = adding_data;
    ...
    e.start_time = parseStartTimeFromDate(data.date, no_time);
...
function addEventSend() {
    var status = editEventBeforeSend(grabCheckboxes(), true, $("#input-add-no_time").prop("checked"));
    if (status[0]) {
        var temp = { action: "add_event", data: {} };
        console.log(adding_data.start_time);
        ...
        temp.data.no_time = 0;
        if ($("#input-add-no_time").prop("checked")) temp.data.no_time = 1;
        var sending_obj = temp;
        if (dates_active > 1) {
            var temps = [temp];
        ...
        sendPostRequest(sending_obj).then(function (res) {
            ...
            if (dates_active > 1) {
                res.data.forEach(function (d) {
                    var e = parseResponse([d.data]);
                    var new_event = new Event(e[0]);
                    events.add(new_event);
                });
            } else {
                var e = parseResponse([res.data]);
                var new_event = new Event(e[0]);
                events.add(new_event);
            }
            invokeCalendarClick();
...
function editEventSend() {
    var status = editEventBeforeSend(grabCheckboxes(true), false, $("#input-edit-no_time").prop("checked"));
    if (status[0]) {
        var temp = { action: "edit_event", data: { id: editing_event.id } };
        ...
        temp.data.no_time = 0;
        if ($("#input-edit-no_time").prop("checked")) temp.data.no_time = 1;

        var buttons = $("button", $("#edit-event"));
        buttons.each(function () {
            $(this).prop("disabled", true);
        });
        sendPostRequest(temp).then(function (res) {
            ...
            editing_event.no_time = res.data.no_time == 1;
            editing_event.start_time = parseDate(res.data.start_time, "decode", res.data.no_time);
...
function moreDates(inc) {
    var $controls = $("#dates-controls");
    var result = '';
    var old_values = [];
    if (inc > 0) {
        dates_active++;
        $controls.html('' + '<button type="button" class="btn btn-primary btn-date-option" onclick="moreDates(1);">Pridať deň</button>' + '<button type="button" class="btn btn-default btn-date-option" onclick="moreDates(-1);">Odobrať deň</button>');
    } else {
        dates_active += inc;
    ...
    var _loop = function _loop(_i) {
        $(function () {
            $('#datetimepicker' + (_i + 1)).datetimepicker({ locale: 'sk', format: 'LLLL' });
        });
        if (old_values[_i]) $("#input-date-" + (_i + 1)).val(old_values[_i]);
    };

    for (var _i = 1; _i < dates_active; _i++) {
        _loop(_i);
    }
}
...
function specialCheckboxes(el) {
    ...
    if (id.indexOf("sport") !== -1) {
        if (sport_selected) hideSport();else showSport();
    } else if (id.indexOf("area") !== -1) {
        if (area_selected) hideArea();else showArea();
    }
}
...
function showDates(i) {
    if (i === 1) {
        $("#add-time-1-div").show();
        $("#add-time-2-div").hide();
        $("#edit-time-1-div").show();
        $("#edit-time-2-div").hide();
    } else {
        $("#add-time-1-div").hide();
        $("#add-time-2-div").show();
        $("#edit-time-1-div").hide();
        $("#edit-time-2-div").show();
    }
}
...
function noTimeClicked(edit) {
    if (edit) {
        if ($("#input-edit-no_time").prop("checked")) showDates(2);
        else showDates(1);
    } else {
        if ($("#input-add-no_time").prop("checked")) showDates(2);
        else showDates(1);
    }
}