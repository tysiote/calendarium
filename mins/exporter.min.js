...
function validateExportOptionsButtons() {
    ...
    setTimeout(function () {
        ...
        $(".dummy-selector-class-checkbox").each(function (i, v) {
            if ($(v).prop("checked")) count++;
        });
        if (count) buttons.forEach(function (b) {
            b.prop("disabled", false);
        });else buttons.forEach(function (b) {
...
function exportMainPressed() {
    ...
    var page_header = $("#daily-header").html();
    if (viewing_mode === "day") page_header = "Export dňa " + page_header.substring(15, page_header.length);else {
        page_header = $("#month-week-title").html();
        page_header = "Export týždňa " + page_header.substring(28, page_header.length);
    }
    var result = '' + '<div class="export-controls">' + '<div class="checkbox checkbox-primary checkbox-inline row-checkbox">' + '<input type="checkbox" value="" id="export-content-check">' + '<label for="export-content-check" class="checkbox-inline" onclick="switchExports();">S popisom</label>' + '</div>' + '<button type="button" class="btn btn-primary" id="btn-export-print" onclick="exportPrint();">Vytlačiť</button>' + '<button type="button" class="btn btn-primary" id="btn-export" onclick="exportTechnical();">Export</button>' + '</div>';
    ...
    selected.forEach(function (r) {
        result += exportOneRow(parseInt(r.prop("id").split("-")[2]), export_type, viewing_mode !== "day");
    });
    ...
    if (LEVEL > 1 && (viewing_mode === "day" || viewing_mode === "week" || viewing_mode === "custom")) $("#btn-export").show();
...
function exportOneRow(id, content, with_day) {
    ...
    result += '<td>' + e.htmlStartTime(true) + '</td><td>' + e.title + '</td>';
    if (content) result += '<td>' + customReplace(e.content, "&nbsp;", " ") + '</td>';
    var tags2 = e.tags2.split("|");
    var res = '';
    tags2.forEach(function (t) {
        res += translateTag(t.toLowerCase()) + ", ";
    });
...
function parseToExport(val) {
    ...
    if (cur + word.length + 1 < max) result += word;else result += "\n" + word;
    result = customReplace(result, "\n", "<br>");
    return result;
}

function exportTechnical() {
    var text = $("#export-body").html();
    text = customReplace(text, "<table>", "");
    text = customReplace(text, "</table>", "");
    text = customReplace(text, "<tbody>", "");
    text = customReplace(text, "</tbody>", "");
    text = findTRs(text, viewing_mode === "week" || viewing_mode === "custom");
...
function replaceTR(tr, week) {
        ...
        if (tr[i] === '<' && i < tr.length - 4 && tr[i + 1] === 't' && tr[i + 2] === 'd' && tr[i + 3] === '>') {
            ...
            if (week) {
                ...
                if (tds === 1) {
                    ...
                    var id = temp.split(" ")[0];
                    var e = events.get(parseInt(id));
                    var d = new Date(e.start_time.year, e.start_time.month - 1, e.start_time.day).getDay();
    ...
    result = customReplace(result, "~|~|~", "<br><br>");
    return result + "<br><br><br>";
}

function exportPrint() {
    var $el = $("body");
    var width = $el.height(window.innerWidth);
    var height = $el.height(window.innerHeight);
    var w = window.open('', 'PRINT', 'height=' + height + ',width=' + width);
    var html = $("#exporting-table").html();
